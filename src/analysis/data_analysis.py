...
try:
    ...
    print("LightGBM bulunamadı. LightGBM modelleri çalıştırılmayacak.")
...
try:
    ...
    print("CatBoost bulunamadı. CatBoost modelleri çalıştırılmayacak.")
...
class ModelAnalyzer:
    ...
    def __init__(self):
        ...
        # AdaBoost ekle
        self.models['AdaBoost'] = AdaBoostClassifier(random_state=42, n_estimators=200, learning_rate=0.05)
    ...
    def evaluate_models(self, X_train, y_train, X_test, y_test):
        """Modelleri değerlendir ve hem train hem test metriklerini hesapla."""
    ...
    def find_best_model(self):
        ...
        for name, metrics in self.results.items():
            # F1-score'u ana kriter olarak kullan (TEST)
    ...
    def plot_roc_curves(self, y_test):
        ...
        for name, metrics in self.results.items():
            if metrics['test']['y_proba'] is not None:
                fpr, tpr, _ = roc_curve(y_test, metrics['test']['y_proba'])
    ...
    def plot_metrics_comparison(self):
        """Model metriklerini karşılaştır (Train/Test ayrı)."""
        rows = []
        for model_name, splits in self.results.items():
            for split_name in ['Train', 'Test']:
                key = 'train' if split_name == 'Train' else 'test'
                rows.append({
                    'Model': model_name,
                    'Split': split_name,
                    'Accuracy': splits[key]['accuracy'],
                    'Precision': splits[key]['precision'],
                    'Recall': splits[key]['recall'],
                    'F1-Score': splits[key]['f1'],
                    'ROC AUC': splits[key]['roc_auc']
                })
        ...
        for idx, metric in enumerate(metrics):
            ...
            # Sadece Test sonuçlarını çubuk grafikte göster (daha anlamlı karşılaştırma için)
            test_df = metrics_df[metrics_df['Split'] == 'Test']
            axes[row, col].bar(test_df['Model'], test_df[metric])
            axes[row, col].set_title(f'{metric} Karşılaştırması (Test)')
...
class ComparativeAnalyzer:
    ...
    def plot_comparative_results(self):
        ...
        # Karşılaştırmalı grafikler (görselleştirme için sadece Test split'ini kullan)
        metrics = ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'ROC AUC']
        ...
        for idx, metric in enumerate(metrics):
            if idx < len(axes):
                # Pivot table oluştur - yalnızca Test split'i
                pivot_data = metrics_comparison[metrics_comparison['Split'] == 'Test'].pivot(index='Model', columns='Data_Type', values=metric)
        ...
        # Sonuçları yazdır
        print("\n=== KARŞILAŞTIRMALI SONUÇLAR TABLOSU (Train/Test Ayrı) ===")
        # Tabloyu Train/Test ve Data_Type ile birlikte yazdır
        ordered_cols = ['Model', 'Split', 'Data_Type', 'Accuracy', 'Precision', 'Recall', 'F1-Score', 'ROC AUC']
        print(metrics_comparison[ordered_cols].to_string(index=False))
