...
class AutoPredictRequest(BaseModel):
    """Otomatik en iyi modeli seç ve risk/öncelik hesabı yap."""
    patient_data: PatientData
    data_path: Optional[str] = None  # yoksa varsayılan eğitim verisi
...
@router.post("/predict/auto", response_model=AutoPredictResponse)
async def auto_predict(request: AutoPredictRequest):
    try:
        # veri yolu
        if request.data_path:
            data_path = str(Path(request.data_path)) if Path(request.data_path).is_absolute() else str(settings.project_root / request.data_path)
        else:
            data_path = str(settings.project_root / settings.data_file)

        result = await prediction_service.auto_select_and_predict(
            data_path=data_path,
            patient_data=request.patient_data.dict(),
            weight_brier=request.weight_brier,
            weight_logloss=request.weight_logloss,
        )

        return AutoPredictResponse(
            success=True,
            message="Otomatik seçim ve tahmin başarıyla tamamlandı",
            data=result,
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Otomatik tahmin hatası: {str(e)}")
...
async def get_available_models():
    ...
    PKL dosyalarından modelleri de otomatik olarak senkronize eder.
    """
    try:
        # Önce PKL dosyalarını veritabanına senkronize et
        sync_result = pkl_model_manager.sync_pkl_to_database()
        print(f"PKL senkronizasyonu: {sync_result['new_models']} yeni, {sync_result['updated_models']} guncellenen")
        
        # Veritabanından modelleri çek
        models = await prediction_service.get_available_models()
        
        return AvailableModelsResponse(
            success=True,
            message=f"{len(models)} kullanılabilir model bulundu (PKL: {sync_result['scanned_files']} dosya)",
            models=models
        )
