...
from api.services.pkl_model_manager import pkl_model_manager
...
async def compare_models(request: ModelComparisonRequest, current_user = Depends(require_admin)):
    """
    ...
    **Çıktılar:**
    ...
    try:
        ...
        # Model karşılaştırmasını çalıştır (tüm modelleri eğitir ve veritabanına kaydeder)
        result = await model_comparison_service.run_model_comparison(data_path)
        
        execution_time = time.time() - start_time
        
        # PKL dosyalarını da senkronize et
        sync_result = pkl_model_manager.sync_pkl_to_database()
        
        return ModelComparisonResponse(
            success=True,
            message=f"Model karşılaştırması başarıyla tamamlandı. {sync_result['new_models']} yeni model veritabanına kaydedildi.",
            data=result,
...
@router.post("/sync-pkl")
async def sync_pkl_models(current_user = Depends(require_admin)):
    """
    PKL dosyalarını veritabanına senkronize et.
    
    Bu endpoint, models/ klasöründeki tüm PKL dosyalarını tarayıp
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"PKL senkronizasyonu tamamlandı",
            "data": {
                "scanned_files": sync_result['scanned_files'],
                "new_models": sync_result['new_models'],
                "updated_models": sync_result['updated_models'],
        ...
        raise HTTPException(
            status_code=500,
            detail=f"PKL senkronizasyon hatası: {str(e)}"
        )

@router.get("/pkl-statistics")
async def get_pkl_statistics():
    """
    PKL dosyaları hakkında istatistikleri döndür.
    """
    try:
        stats = pkl_model_manager.get_model_statistics()
        
        return {
            "success": True,
            "message": "PKL istatistikleri alındı",
            "data": stats
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"PKL istatistik hatası: {str(e)}"
        )
...
async def get_model_metrics(request: MetricsRequest):
    ...
    try:
        ...
        metrics = await model_comparison_service.evaluate_model_metrics(data_path, n_bins=request.n_bins)

        return MetricsResponse(
            success=True,
            message="Metrikler başarıyla hesaplandı",
            data=metrics,
            execution_time=time.time() - start_time
        )
    except Exception as e:
...
@router.post("/best", response_model=BestModelResponse)
async def select_best_model(request: BestModelRequest):
    ...
    try:
        ...
        selection = await model_comparison_service.select_best_model_by_calibration(data_path, n_bins=request.n_bins)

        return BestModelResponse(
            success=True,
            message="En iyi model seçimi başarıyla tamamlandı",
            data=selection,
            execution_time=time.time() - start_time
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"En iyi model seçimi hatası: {str(e)}")
